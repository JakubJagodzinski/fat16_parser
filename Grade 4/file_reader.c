...
void *find_dir_or_file_by_path(struct volume_t *pvolume, const char *path, enum seek_type_t seek_type) {
    ...
    do {
        ...
        while (pdir->position < pdir->capacity) {
            if (*(pdir->info + pdir->position)->name == '\0' || *(pdir->info + pdir->position)->name == FAT_DIR_DELETED || ((pdir->info + pdir->position)->attributes & FAT_ATTRIBUTE_VOLUME_LABEL) == FAT_ATTRIBUTE_VOLUME_LABEL) {
                ++pdir->position;
                continue;
            }
            if (ignore_case_strcmp(fat_dir_info_name, (pdir->info + pdir->position)->name, FAT_DIR_INFO_NAME_LEN) == 0) {
                ...
                if (seek_type == SEEK_FILE && next == NULL) {
                    if (((pdir->info + pdir->position)->attributes & FAT_ATTRIBUTE_DIRECTORY) == FAT_ATTRIBUTE_DIRECTORY) {
                        free(path_copy);
                        dir_close(pdir);
                        errno = EISDIR;
